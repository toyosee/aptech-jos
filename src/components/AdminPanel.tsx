...
export default function AdminPanel() {
  // FIX 1: Session Persistence via LocalStorage
  const [isAuth, setIsAuth] = useState(() => {
    return localStorage.getItem('aptech_admin_auth') === 'true';
  });
  ...
  const handleLogin = () => {
    if (pass === 'admin2026@@##') {
      ...
      toast.error('Invalid Secure Key');
    }
  };

  const handleSignOut = () => {
    setIsAuth(false);
    localStorage.removeItem('aptech_admin_auth');
  ...
  return (
    <div className="flex min-h-screen bg-[#fefdfd] font-sans text-[#070504]">
      ...
      <aside className="w-72 bg-[#070504] flex flex-col hidden lg:flex border-r border-white/5">
        ...
        <div className="p-6 border-t border-white/10">
          <button onClick={handleSignOut} className="flex items-center gap-3 text-[#9f8580] hover:text-[#ea282c] transition-colors px-4 py-2 w-full font-bold uppercase text-xs tracking-widest">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* Header */}
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 md:px-8 z-10 shrink-0">
          <div className="flex items-center gap-4 bg-slate-50 px-4 md:px-5 py-2 rounded-2xl w-full max-w-xs md:max-w-md border border-slate-100">
            <Search size={16} className="text-[#9f8580]" />
            <input 
               value={search} 
               onChange={(e) => setSearch(e.target.value)} 
               placeholder="Search..." 
               className="bg-transparent border-none outline-none text-xs md:text-sm w-full font-bold placeholder:text-[#9f8580]" 
            />
          </div>
          <div className="flex items-center gap-4">
             {activeTab === 'records' && (
               <button 
                 onClick={exportToCSV}
                 className="hidden md:flex items-center gap-2 px-5 py-2.5 bg-[#dda816] text-[#070504] rounded-xl text-sm font-black hover:brightness-105 transition shadow-lg shadow-yellow-100 active:scale-95 uppercase tracking-tight"
               >
        ...
        {/* Content Area - Adjusted padding for mobile bottom nav */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50/50 pb-24 lg:pb-8">
          {activeTab === 'dash' && (
            <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-3 duration-500">
              <header>
                <h1 className="text-2xl md:text-3xl font-black text-[#070504] tracking-tight uppercase">Overview</h1>
                <p className="text-[#9f8580] font-medium text-sm">Real-time stats for Aptech Jos.</p>
              </header>
          ...
          {activeTab === 'records' && (
            <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-3 duration-500">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black text-[#070504] uppercase tracking-tight">Records</h2>
                <button onClick={exportToCSV} className="md:hidden p-2 bg-[#dda816] rounded-lg text-[#070504]"><Download size={20}/></button>
              </div>
              
              {filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-20 bg-white rounded-[32px] border border-slate-200">
                  <Inbox size={48} className="text-[#9f8580] opacity-20 mb-4" />
                  <p className="text-[#9f8580] font-bold uppercase tracking-widest text-xs text-center">No matches found in database</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left min-w-[600px]">
                    <thead className="bg-[#070504] text-white">
                      <tr>
                        ...
                        <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em]">Course</th>
                        <th className="p-6"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {filtered.map(user => (
                        <tr key={user.$id} className="hover:bg-slate-50 transition-colors">
                          <td className="p-6 font-black text-[#070504] text-sm">{user.firstName} {user.lastName}</td>
                          <td className="p-6">
                            <div className="text-xs font-bold text-[#ea282c] truncate max-w-[150px]">{user.email}</div>
                            <div className="text-[10px] font-medium text-[#9f8580]">{user.phoneNumber}</div>
                          </td>
                          <td className="p-6">
                            <span className="px-3 py-1 bg-slate-100 text-[#070504] rounded-lg text-[9px] font-black uppercase tracking-wider border border-slate-200">
                              {user.course}
                            </span>
                          </td>
          ...
          {activeTab === 'analytics' && (
            <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-3 duration-500">
              <h1 className="text-2xl md:text-3xl font-black text-[#070504] tracking-tight uppercase">Analytics</h1>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 ...
                 <div className="bg-[#070504] p-10 rounded-[40px] flex flex-col justify-center items-center text-center text-white relative overflow-hidden min-h-[300px]">
                    ...
                    <p className="text-[#9f8580] mt-4 font-medium leading-relaxed text-sm">
                        Registration data shows a high demand for <span className="text-white">Professional Courses</span> this quarter.
                    </p>
                 </div>
              </div>
            </div>
        ...
        {/* FIX 2: Mobile Bottom Navigation Bar */}
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#070504] border-t border-white/10 flex justify-around p-4 z-50">
          <MobileNavBtn icon={<LayoutDashboard size={20}/>} label="Dash" active={activeTab === 'dash'} onClick={() => setActiveTab('dash')} />
          <MobileNavBtn icon={<Users size={20}/>} label="Users" active={activeTab === 'records'} onClick={() => setActiveTab('records')} />
          <MobileNavBtn icon={<BarChart3 size={20}/>} label="Stats" active={activeTab === 'analytics'} onClick={() => setActiveTab('analytics')} />
          <button onClick={handleSignOut} className="flex flex-col items-center text-[#9f8580]">
            <LogOut size={20} />
...
const MobileNavBtn = ({ icon, label, active, onClick }: MobileNavBtnProps) => (
  <button onClick={onClick} className={`flex flex-col items-center transition-colors ${active ? 'text-[#ea282c]' : 'text-[#9f8580]'}`}>
    {icon}
...
const StatCard = ({ title, value, trend, icon }: StatCardProps) => (
  <div className="bg-white p-6 md:p-8 rounded-[32px] border border-slate-200 shadow-sm hover:border-[#ea282c] transition-all group">
    ...
    <h3 className="text-4xl md:text-5xl font-black text-[#070504] tracking-tighter">{value}</h3>
    <div className="mt-6 text-[#070504] font-black text-[9px] bg-[#dda816] w-fit px-4 py-1.5 rounded-full uppercase tracking-widest italic">
      {trend}
    </div>
  </div>
);